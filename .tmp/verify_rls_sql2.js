...
async function run() {
  ...
  // What if we don't request exact count?
  const { data, error: uErr } = await supabase.from('sessions').update({ time_spent: 500 }).eq('id', sessId);
  console.log("Update Error:", uErr);
  
  // Can we update it again with count to see if it worked? No, count requires SELECT.
}
run();
